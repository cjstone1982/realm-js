...
function optionalPropertyNames(schema) {
    return Object.keys(schema.properties).filter((name) => name.includes('opt') || name == 'objectCol');
}
function nonoptionalPropertyNames(schema) {
    return Object.keys(schema.properties).filter((name) => !name.includes('opt'));
}
function scalarPropertyNames(schema) {
    return Object.keys(schema.properties).filter((name) => !name.includes('Array'));
}
function arrayPropertyNames(schema) {
    return Object.keys(schema.properties).filter((name) => name.includes('Array'));
}
...
module.exports = {
    ...
    testAllTypesPropertySetters: function() {
        ...
        realm.write(function() {
            ...
            for (const name of nonoptionalPropertyNames(schemas.AllTypes)) {
            ...
            for (const name of optionalPropertyNames(schemas.AllTypes)) {
                obj[name] = null;
            ...
            function tryAssign(name, value) {
                ...
                var type = typeof prop == 'object' ? prop.type : prop;
                obj[name] = value;
                assertEqual(type, obj[name], value, 1);
            }
